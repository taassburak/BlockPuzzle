...
public class CrucialColliderGizmoEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        target.selectedPreset = (CCGPresets) EditorGUILayout.EnumPopup(new GUIContent("Color Preset", "Presets color profiles that, when selected, will update your colors to preset values."),
        ...
        if (target.selectedPreset != oldPreset)
        {
            ...
            switch (target.selectedPreset)
            {
                ...
                case CCGPresets.Green:
                    target.wireColor = new Color(.5f, 1.0f, .5f, .3f);
                    target.fillColor = new Color(.1f, 1.0f, .1f, .3f);
                    target.centerColor = new Color(.5f, 1.0f, .5f, .5f);
                    break;
        ...
        target.edgePointMarkerRadius =
            EditorGUILayout.FloatField(new GUIContent("Edge Collider Marker Radius", "The radius of the collider marker on your collider(s)."), target.edgePointMarkerRadius);
        ...
        target.collider2D_ZDepth =
            EditorGUILayout.FloatField(
                new GUIContent("BoxCollider2D Z Depth",
                    "The thickness, or Z axis length, of your 2D box collider visualizations. This is just to help assist in 3D visualization of 2D colliders, as the visual thickness of this representation will not reflect any change in the collider's behavior at runtime."),
                target.collider2D_ZDepth);
